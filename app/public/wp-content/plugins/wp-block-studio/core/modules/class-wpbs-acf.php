...
class WPBS_ACF_field_select_social extends \acf_field {

    public $show_in_rest = true;

    public function __construct() {

        $this->name     = 'social_select';
        $this->label    = __( 'Social Select', 'WPBS' );
        $this->category = 'choice'; // basic | content | choice | relational | jquery | layout | CUSTOM GROUP NAME
        $this->supports = [
                'escaping_html' => true
        ];
    ...
    public function render_field( $field ): void {
        ...
        >
            ...
            <?php foreach ( $services as $slug => $service ) {
                ...
                if ( empty( $service['name'] ) || empty( $slug ) ) {
                    continue;
                }
                ...
                <option value="<?= $slug; ?>"
                        <?php echo ( $field['value'] ?? false ) == $slug ? 'selected' : null ?>
                >
...
class WPBS_ACF {
    ...
    public function register_fields(): void {
        ...
        acf_register_field_type( 'wpbs_acf_field_select_social' );
    }
